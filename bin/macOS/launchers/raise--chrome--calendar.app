...
tell application "Google Chrome"
  repeat with w in windows
    tell w
      if title of w starts with "Calendar - " then
        set index to 1
  ...
  # At one client, when you join a meeting from the calendar entry, it
  # opens a new Webex tab. So sometimes the active tab is the Webex tab,
  # and not the calendar tab.
  repeat with w in windows
    tell w
      if title of w starts with "Cisco Webex Meetings - " then
        set index to 1
        tell application "System Events" to tell process "Google Chrome"
          perform action "AXRaise" of window 1
          set frontmost to true
